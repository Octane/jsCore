// simple fadeId/fadeOut effects
core.prototype.fadeOut = core.ie ? function() {
	// for using AlphaImageLoader and png-24
	return this.hide();
} : function() {
	var opacity = 1;
	new core.timer(30, function() {
...
core.prototype.fadeIn = core.ie ? function() {
	// for using AlphaImageLoader and png-24
	return this.show();
} : function() {
	var opacity = .2;
...
$.ready(function() {
	...
	$('header').exist(function() {
		...
		var show = function() {
			...
			tooltip.html(this.tooltip).fadeIn();
	...
	(function(button, dom, javascript, actions) {
		...
		button.click(function() {
			if(state < 3) {
				...
				actions.item(state).fadeIn('list-item');
	...
	if($.ie == 6) $.tags('button').each(function() {
		$(this).mouseover('addClass', 'hover').mouseout('removeClass', 'hover').focus('addClass', 'hover').blur('removeClass', 'hover').mousedown('addClass', 'active').mouseup('removeClass', 'active');
	});
